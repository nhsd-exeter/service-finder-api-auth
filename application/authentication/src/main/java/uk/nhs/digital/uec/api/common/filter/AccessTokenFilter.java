...
import uk.nhs.digital.uec.api.common.request.ServiceFinderRequest;
...
import static uk.nhs.digital.uec.api.common.UserManagementCommonConstants.IDENTITY_PROVIDER_ID;
import static uk.nhs.digital.uec.api.common.UserManagementCommonConstants.SUB;
import static uk.nhs.digital.uec.api.common.UserManagementCommonConstants.USER_HASH;
import static uk.nhs.digital.uec.api.common.util.CookieUtil.getCookieValue;

/**
 * A web filter responsible for reading and processing the access token in the request. It adds roles to the {@link Authentication} object previously set by the
 * Spring {@code OAuth2AuthenticationProcessingFilter}, and adds the identity provider ID to the request as a request attribute. Cognito groups in the JWT are
 * mapped directly to roles; the Cognito 'sub' is used as the identity provider ID.
 */
public class AccessTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        serviceFinderRequest.setAuthenticationToken(CookieTokenExtractor.extractAuthenticationToken(request));
    ...
    private Authentication createNewAuthentication(Authentication origAuthentication, String accessToken) {
        ...
        return new UsernamePasswordAuthenticationToken(
            origAuthentication.getPrincipal(),
            origAuthentication.getDetails(),
            authorities
        );
    }
    ...
    private List<GrantedAuthority> convertCognitoGroupsToAuthorities(List<String> groupList) {
        ...
        groupList.forEach(cognitoGroupName -> {
