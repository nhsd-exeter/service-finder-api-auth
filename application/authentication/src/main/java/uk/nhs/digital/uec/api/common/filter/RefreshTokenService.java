...
public class RefreshTokenService {
     ...
     * @throws IllegalStateException if {@link LoginResult} is null or access or refresh tokens are blank
     */
    public LoginResult refresh(String refreshToken, String identityProviderId) {
        ...
        LoginResult loginResult = restTemplate.postForObject(userManagementUrl + REFRESH_PATH, refreshTokens, LoginResult.class);
        if (loginResult == null || StringUtils.isBlank(loginResult.getAccessToken()) || StringUtils.isBlank(loginResult.getRefreshToken())) {
            throw new IllegalStateException("Unexpected state: null detected on loginResult / accessToken / refreshToken");
        }
        return loginResult;
    }
