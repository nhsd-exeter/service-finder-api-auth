...
SELECT
  ...
  u.updated,
  null,
  -- this is initially null, because we may not have inserted the updated_by user yet.
  ...
  u.user_state
FROM
  service_finder."user" u
WHERE
  NOT EXISTS (
    SELECT
      1
    FROM
      service_finder."user" u2
    WHERE
      lower(u2.email_address) = lower(u.email_address)
      AND u2.id < u.id
  )
ORDER BY
  u.id ASC;
...
FROM
  ...
  service_finder."user_account" ua
WHERE
  ua.email_address = u.email_address
  AND NOT EXISTS (
    SELECT
      1
    FROM
...
SELECT
  setval(
    'service_finder.user_details_id_seq',
    (
      SELECT
        MAX(id)
      FROM
...
-- Insert users that are in Cognito but not service finder, do not re-introduce duplicates.
INSERT INTO
  service_finder.user_account (
    identity_provider_id,
    email_address,
...
SELECT
  ...
  created_date
FROM
  service_finder.user_compare_tmp uc
WHERE
  NOT EXISTS (
    SELECT
      1
    FROM
      service_finder.user_account ua
    WHERE
      lower(ua.email_address) = lower(uc.email_address)
  );
